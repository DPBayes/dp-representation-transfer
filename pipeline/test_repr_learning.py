...
alg = (
  "dummy",
  "Dummy (assume everything is zero)",
  lambda input_dim, repr_dim:
    __import__('models.dummy').dummy.Dummy().init(
      input_dim = input_dim,
      output_dim = repr_dim)
)
  ...
  alg = (
    ...
    lambda input_dim, repr_dim, optimizer=optimizer:
      __import__('models.ae').ae.AE().init(
        input_dim = input_dim,
        #enc_dims = [np.rint(expInterp(input_dim, repr_dim, 0.5))],
        enc_dims = [4*input_dim, 2*input_dim],
        output_dim = repr_dim,
        dec_dims = "same",
        enc_activations = ['prelu', 'prelu', 'prelu'],
...
alg = (
  ...
  lambda input_dim, repr_dim:
    __import__('models.ae').ae.AE().init(
      input_dim = input_dim,
      enc_dims = [input_dim, np.rint(expInterp(input_dim, repr_dim, 0.5))],
      output_dim = repr_dim,
      dec_dims = "same",
      enc_activations = ['softplus', 'softplus', 'softplus'],
      dec_activations = ['softplus', 'softplus', 'tanh'],
      dropout = 0.2,
      n_epochs = n_epochs,
      batch_size = 100)
)
...
#  use = True
#  name = "pca"-m pdb
#  desc = "PCA (principal component analysis)"
#  def make(input_dim, repr_dim):
#    from models.pca import PCA
#    return PCA().init(
#      input_dim = input_dim,
#      output_dim = repr_dim)
#algorithms.append(AlgType())
#del AlgType()
