...
def read_data_HDF5(filename, rows=slice(None), cols=slice(None)):

  #open file for read-only
  with h5py.File(filename, 'r') as f:
    data_obj = f.get('Data')
    #data = np.empty(data_obj.shape, dtype='float32')
    #data_obj.read_direct(data, (rows, cols1))
    with data_obj.astype('float32'):
      data = data_obj[rows, cols]
  return data
