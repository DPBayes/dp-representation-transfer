...
# Two layer AE
#for optimizer in ['Adagrad', 'Adam', 'Adamax', 'Nadam']:
#  alg = (
#    "ae1_2x_" + optimizer,
...
#for optimizer in ['Adam']:
  ...
  #for first_hidden_dim_mult in [8]:
    alg = (
      "ae7_%sxR_dropout25_%s" % (max_hidden_dim_mult, optimizer),
      "Four layer autoencoder",
      lambda input_dim, repr_dim,
             optimizer=optimizer, max_hidden_dim_mult=max_hidden_dim_mult:
        __import__('models.ae').ae.AE().init(
          input_dim = input_dim,
          enc_dims = [a*repr_dim for a in [max_hidden_dim_mult, max_hidden_dim_mult, max_hidden_dim_mult, max(max_hidden_dim_mult, 16), 8, 4, 2]],
          output_dim = repr_dim,
          dec_dims = "same",
          enc_activations = ['prelu' for i in range(7)] + ['linear'],
          dec_activations = ['prelu' for i in range(7)] + ['linear'],
          dropout=[0.2] +[0.5 for i in range(7)],
          n_epochs = n_epochs,
          batch_size = batch_size,
          optimizer=optimizer,
          log_weights=log_weights,
    ...
    algorithms.append(alg)
    alg = (
      "ae7_%sxR_dropout12_%s" % (max_hidden_dim_mult, optimizer),
      "Four layer autoencoder",
      lambda input_dim, repr_dim,
             optimizer=optimizer, max_hidden_dim_mult=max_hidden_dim_mult:
        __import__('models.ae').ae.AE().init(
          ...
          enc_activations = ['prelu' for i in range(7)] + ['linear'],
          dec_activations = ['prelu' for i in range(7)] + ['linear'],
          dropout=[0.1] +[0.2 for i in range(7)],
          n_epochs = n_epochs,
          batch_size = batch_size,
          optimizer=optimizer,
          log_weights=log_weights,
    ...
    algorithms.append(alg)
    alg = (
      "ae7_%sxR_batchnorm_%s" % (max_hidden_dim_mult, optimizer),
      "Four layer autoencoder",
      lambda input_dim, repr_dim,
             optimizer=optimizer, max_hidden_dim_mult=max_hidden_dim_mult:
        __import__('models.ae').ae.AE().init(
...
def task(args):
  ...
  # load gene names
  #features = readHDF5.getHDF5data("data/geneNames.h5", True, False)[0]
  ...
  progress_filename = 'res/progress-encdec-mse-%s-%s-%d-%s.txt' % (data_set, input_dim, repr_dim, algName)
  progress_file = open(progress_filename, 'w', encoding='utf-8')
  if val_x is not None:
    val_progress_filename = 'res/progress-encdec-validation-mse-%s-%s-%d-%s.txt' % (data_set, input_dim, repr_dim, algName)
  ...
  alg.learn(x,
            log_file_prefix=("log/%s-%s-%d-%s" % (data_set, input_dim, repr_dim, algName)),
            per_epoch_callback_funs=[save_progress], callbacks=callbacks)
  
  # test fitting
  x_pred = alg.decode(alg.encode(x))
  ensure_dir_exists('pred')
  pred_filename = 'pred/final-encdec-%s-%s-%d-%s' % (data_set, input_dim, repr_dim, algName)
  if save_pred:
  ...
  res_filename = 'res/final-encdec-mse-%s-%s-%d-%s.txt' % (data_set, input_dim, repr_dim, algName)
