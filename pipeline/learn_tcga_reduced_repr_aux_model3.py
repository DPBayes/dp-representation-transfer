...
hidden_dim_mult = 64
alg = (
  "ae1_%sxR_dropout%d%d_%s"  % (hidden_dim_mult, 10*first_layer_dropout, 10*other_layer_dropouts, optimizer),
...
alg = (
  "ae2_%sxR_dropout%d%d_%s"  % (first_hidden_dim_mult, 10*first_layer_dropout, 10*other_layer_dropouts, optimizer),
...
first_hidden_dim_mult = 32
alg = (
  "ae3_%sxR_dropout%d%d_%s"  % (first_hidden_dim_mult, 10*first_layer_dropout, 10*other_layer_dropouts, optimizer),
...
for prediction_var in ['gs', 'gi', 'ps', 'pi']:
  first_hidden_dim_mult = 64
  alg = (
    "vae_%s_e%d_%s" % (prediction_var, first_hidden_dim_mult, optimizer),
    ...
    lambda input_dim, repr_dim,
      ...
      __import__('models.vae').vae.VAE().init(
        input_dim = input_dim,
        latent_dim = repr_dim,
        enc_dims = [first_hidden_dim_mult*repr_dim],
        dec_dims = 'same',
        enc_activations = 'prelu',
        dec_activations = 'prelu',
        prediction_var = prediction_var,
        prediction_log_var_min = np.log(0.1**2),
        input_dropout = first_layer_dropout,
        enc_dropout = other_layer_dropouts,
        optimizer = optimizer,
  ...
  first_hidden_dim_mult = 32
  alg = (
    "vae_%s_e%de4_%s" % (prediction_var, first_hidden_dim_mult, optimizer),
    "Variational autoencoder",
    lambda input_dim, repr_dim,
           prediction_var=prediction_var,
           optimizer=optimizer, first_hidden_dim_mult=first_hidden_dim_mult:
