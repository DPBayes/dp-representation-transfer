...
test_params = SimpleNamespace(
  ...
  #priv_cancertypes = cancer_type_pairs[0], val_cancertypes = cancer_type_pairs[1],
  #priv_cancertypes = cancer_type_pairs[0], val_cancertypes = cancer_type_pairs[2],
  #priv_cancertypes = cancer_type_pairs[1], val_cancertypes = cancer_type_pairs[0],
  #priv_cancertypes = cancer_type_pairs[1], val_cancertypes = cancer_type_pairs[2],
  #priv_cancertypes = cancer_type_pairs[2], val_cancertypes = cancer_type_pairs[0],
  #priv_cancertypes = cancer_type_pairs[2], val_cancertypes = cancer_type_pairs[1],
  #priv_cancertypes = cancer_type_pairs[0], val_cancertypes = cancer_type_pairs[3],
  #priv_cancertypes = cancer_type_pairs[3], val_cancertypes = cancer_type_pairs[0],
  #priv_cancertypes = cancer_type_pairs[1], val_cancertypes = cancer_type_pairs[3],
  priv_cancertypes = cancer_type_pairs[3], val_cancertypes = cancer_type_pairs[1],
  #priv_cancertypes = cancer_type_pairs[2], val_cancertypes = cancer_type_pairs[3],
  #priv_cancertypes = cancer_type_pairs[3], val_cancertypes = cancer_type_pairs[2],

  #priv_cancertypes = ["lung squamous cell carcinoma", "head & neck squamous cell carcinoma"], val_cancertypes = ["kidney clear cell carcinoma", "kidney papillary cell carcinoma"],

  #priv_cancertypes = cancer_type_pairs[5], val_cancertypes = cancer_type_pairs[4],
...
gpyopt_max_duration = datetime.timedelta(hours=60)
...
def select_repr_alg(repr_alg):
  if repr_alg == "VAE":
    ...
    opt_param_domain = [
      ...
      #{'name': 'input_dropout', 'type': 'continuous', 'domain': (0.0, 0.8)},
      #{'name': 'enc_dropout', 'type': 'continuous', 'domain': (0.0, 0.8)},
      #{'name': 'dec_dropout', 'type': 'continuous', 'domain': (0.0, 0.8)},
      ]
...
def run_test(args, fixed_params, task_param, seeds, slurm_args=None):
  ...
  for param, seed, param_id in zip(task_params, seeds, param_ids):
    param.param_id = param_id
    #param.priv_cancertypes = priv_cancertypes
    #param.skip_cancertypes = []
    param.seed = seed
  ensure_dir_exists("run_parameters")
  #np.save("run_parameters/params-%s.npy" % (test_name), params)
  common_params = SimpleNamespace(
...
def main(args):
    data_name = (('-'.join(['priv',] + test_params.priv_cancertypes +
                  ['val',] + test_params.val_cancertypes))
                  .replace(' ', '_').replace('&', '_'))
