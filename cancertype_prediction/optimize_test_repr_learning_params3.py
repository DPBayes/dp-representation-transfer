...
test_params = SimpleNamespace(
  ...
  priv_cancertype_pair = cancer_type_pairs[0],
  param_opt_folds = 2,
...
def select_repr_alg(repr_alg):
  if repr_alg == "VAE_hyper":
    ...
    opt_param_domain = [
      ...
      {'name': 'prediction_var', 'type': 'discrete', 'domain': [0, 1]},
      {'name': 'prediction_log_var_min', 'type': 'continuous', 'domain': (-20,0)},
      {'name': 'activation', 'type': 'discrete', 'domain': [0, 1]},
      {'name': 'normalize_input_quantile', 'type': 'continuous', 'domain': (0.0, 0.4)},
      {'name': 'input_dropout', 'type': 'continuous', 'domain': (0.0, 0.9)},
      {'name': 'enc_dropout', 'type': 'continuous', 'domain': (0.0, 0.9)},
      {'name': 'dec_dropout', 'type': 'continuous', 'domain': (0.0, 0.9)},
      {'name': 'batch_size', 'type': 'discrete', 'domain': [0, 1, 2, 3]},
      ]
    ...
    def make_alg(data_dim, repr_dim, params):
      ...
      alg = VAE().init(
        ...
        dec_dims = 'same',
        enc_activations = ['relu', 'prelu'][int(params.activation)],
        dec_activations = ['relu', 'prelu'][int(params.activation)],
        prediction_mean_activation = 'sigmoid',
        prediction_var = ['gs', 'gi'][int(params.prediction_var)],
        prediction_log_var_min = float(params.prediction_log_var_min),
        normalize_input_type = 'quantiles',
        normalize_input_quantile = params.normalize_input_quantile,
        ...
        batch_size = [16, 32, 64, 128][int(params.batch_size)])
      return alg
...
def run_optimization_batch(args, fixed_params, task_params, seeds, slurm_args=None):
  ...
  nonpriv_cancertype_pairs = [ctp for ctp in cancer_type_pairs
  ...
  for fold in range(fixed_params.param_opt_folds):
    val_cancertype_pairs = [ctp for (i, ctp) in enumerate(nonpriv_cancertype_pairs)
                            if i % fixed_params.param_opt_folds == fold]
    learn_cancertype_pairs = [ctp for (i, ctp) in enumerate(nonpriv_cancertype_pairs)
                              if i % fixed_params.param_opt_folds != fold]
    assert (len(val_cancertype_pairs) + len(learn_cancertype_pairs) ==
            len(nonpriv_cancertype_pairs))
...
def main(args):
    data_name = (('-'.join(['priv',] + test_params.priv_cancertype_pair))
                  .replace(' ', '_').replace('&', '_'))
