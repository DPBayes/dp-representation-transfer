...
def create_activation(name):
  if name.lower() == 'ReLU'.lower():
    ...
    return nn.ELU()
  elif name.lower() == 'sigmoid'.lower():
    return nn.Sigmoid()
  elif name.lower() == 'tanh'.lower():
    return nn.Tanh()
  elif name.lower() == 'softplus'.lower():
    return nn.Softplus()
  elif name.lower() == 'softmax'.lower():
    return nn.Softmax()
  else:
    assert False
...
class VAE:
  def init(self,
           ...
           prediction_log_var_activation=None,
           ...
           normalize_input_type=None,      # one of: None
                                           #         'mean', 'median', 'min', 'max',
                                           #         'stddev', 'quantiles', 'minmax'
           normalize_input_axis='global',  # one of: 'global', 'feature_wise'
           normalize_input_target=(-1, 1), # either a scalar (if normalizing
                                           # mean/median/min/max) or a tuple of two
                                           # scalars (if normalizing
                                           # stddev/quantiles/minmax)
           normalize_input_quantile=0.25,  # used only if normalizing quantiles
           normalize_input_stddev_mult=1,  # used only if normalizing stddev
           normalize_input_clip=False,     # clip after normalization to target
           #dropout=0,
           input_dropout=0,
           enc_dropout=0,
           latent_dropout=0,
    ...
    self.params.normalize_input_type = normalize_input_type
    self.params.normalize_input_axis = normalize_input_axis
    self.params.normalize_input_target = normalize_input_target
    self.params.normalize_input_quantile = normalize_input_quantile
    self.params.normalize_input_stddev_mult = normalize_input_stddev_mult
    self.params.normalize_input_clip = normalize_input_clip
    ...
    self.params.prediction_mean_activation = prediction_mean_activation
    self.params.prediction_log_var_activation = prediction_log_var_activation
    ...
    self.normalizer = Normalizer().init(**(self.params.__dict__))
    ...
    class VAEModel(nn.Module):
      ...
      def decode(self, z):
        ...
        if self.dec_x_log_var_act:
          decoded_x_log_var = self.dec_x_log_var_act(decoded_x_log_var)
  ...
  def learn(self, x,
    ...
    for epoch in range(self.params.n_epochs):
      ...
      print_err("Epoch: %4d  Avg. loss: %.3f" % (
      ...
      if validation:
        ...
        for batch_idx, batch_rows in enumerate(batches):
          ...
          log_prob += torch.sum(gaussian_log_prob(batch_data, recon_mean, recon_log_var)).data[0]
          kl_dist += torch.sum(std_gaussian_kl_dist(z_mean, z_log_var)).data[0]
        
        val_loss /= validation_data.shape[0]
        log_prob /= validation_data.shape[0]
        kl_dist /= validation_data.shape[0]
        print_err("  Val. loss: %.3f (re: %.3f, kl: %.3f)" % (val_loss, log_prob, kl_dist),
                  end='', flush=True)
  ...
  def decode(self, z):
    ...
    x = x_mean.data.cpu().numpy()
    return self.normalizer.restore(x)
  ...
  def save(self, filename):
    ...
    self.normalizer.save(filename + "_normalizer.pkl")
  ...
  def load(self, filename):
    ...
    self.normalizer = Normalizer().load(filename + "_normalizer.pkl")
